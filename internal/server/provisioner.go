...
// RestartWorkspace restarts a workspace asynchronously with distributed locking
func (p *ProvisionerService) RestartWorkspace(ctx context.Context,
	req *provisionerpb.Workspace) (*provisionerpb.RestartWorkspaceResponse, error) {
	...
	if err != nil {
		if errors.Is(err, ws.ErrLockAlreadyHeld) {
			return &provisionerpb.RestartWorkspaceResponse{
				Message: fmt.Sprintf("Request to restart the workspace %s already exists", name),
			}, nil
		}
		return nil, status.Errorf(codes.Internal,
			"Failed to acquire lock for workspace %s restart: %v", name, err)
	}
	...
	go func() {
		defer func() {
			...
			if unlockErr := workspaceLock.Release(unlockCtx); unlockErr != nil {
				p.log.Error().Err(unlockErr).Msgf("Failed to release lock after restarting workspace %s", name)
			}
		}()

		ctx := context.Background()
		time.Sleep(2 * time.Second)
		p.log.Debug().Msgf("Starting async restart of workspace %s", name)

		err := w.DeletePod(ctx)
		if err != nil {
			p.log.Error().Err(err).Msgf("Failed to delete pod for workspace %s during restart", name)
			return
		}

		opts := &ws.ProvisionOptions{
			Timeout:     20,
			Messages:    nil,
			LockTimeout: 30,
		}

		status, err := w.Upgrade(ctx, opts)
		if err != nil {
			p.log.Error().Err(err).Msgf("Failed to restart workspace %s", name)
			return
		}

		p.log.Info().Msgf("Successfully restarted workspace %s: %s - %s", name, status.Status, status.Message)
	}()

	return &provisionerpb.RestartWorkspaceResponse{
		Message: fmt.Sprintf("Request to restart the workspace %s was submitted", name),
	}, nil
}
